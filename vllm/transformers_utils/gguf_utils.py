...
def maybe_patch_hf_config_from_gguf(
    ...
    # Patch qwen35 MTP (Multi-Token Prediction) and SSM (State Space Model) config
    # Read GGUF metadata for MTP/SSM configuration
    if not check_gguf_file(model):
        return hf_config
    try:
        import gguf
        reader = gguf.GGUFReader(str(model))
        # Patch MTP (Multi-Token Prediction) config
        mtp_field = reader.get_field("qwen35.nextn_predict_layers")
        if mtp_field is not None:
            mtp_layers = int(mtp_field.parts[-1])
            # Add MTP config to hf_config if not already present
            if not hasattr(hf_config, 'mtp_num_hidden_layers') or hf_config.mtp_num_hidden_layers is None:
                hf_config.mtp_num_hidden_layers = mtp_layers
                logger.info("Patched qwen35 MTP config: mtp_num_hidden_layers=%d", mtp_layers)
        # Patch SSM (State Space Model) config
        ssm_fields = {
        ...
        for gguf_key, config_key in ssm_fields.items():
            ssm_field = reader.get_field(gguf_key)
            if ssm_field is not None:
                value = int(ssm_field.parts[-1])
                if not hasattr(hf_config, config_key) or getattr(hf_config, config_key, None) is None:
                    setattr(hf_config, config_key, value)
                    logger.info("Patched qwen35 SSM config: %s=%d", config_key, value)
    except Exception as e:
        logger.warning("Failed to patch GGUF config for qwen35: %s", e)
    return hf_config
