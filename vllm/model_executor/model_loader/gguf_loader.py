...
class GGUFModelLoader(BaseModelLoader):
    ...
    def _get_gguf_weights_map(self, model_config: ModelConfig):
        ...
        if model_type in ("qwen2_moe", "qwen3_moe", "qwen3_5", "qwen3_5_text", "qwen3_5_moe", "qwen3_5_moe_text"):
            # Map HuggingFace model_type to GGUF MODEL_ARCH_NAMES key
            model_type = model_type.replace("_", "")
            # qwen3_5_text -> qwen35, qwen3_5_moe_text -> qwen35moe
            model_type = model_type.replace("qwen35text", "qwen35")
            model_type = model_type.replace("qwen35moetext", "qwen35moe")
            # GGUF layer map assumes that we will have a merged expert weights
            # so we need to map them manually
            for idx in range(config.num_hidden_layers):
                gguf_to_hf_name_map[f"blk.{idx}.ffn_down_exps.weight"] = (
        ...
        # qwen35 has SSM (State Space Model) and MTP (Multi-Token Prediction) layers
        if model_type == "qwen35":
            # SSM tensor mappings (blk.N.ssm_*)
            for idx in range(config.num_hidden_layers):
                gguf_to_hf_name_map[f"blk.{idx}.ssm_a"] = (
                    f"model.layers.{idx}.ssm_a"
                )
                gguf_to_hf_name_map[f"blk.{idx}.ssm_alpha.weight"] = (
                    f"model.layers.{idx}.ssm_alpha.weight"
                )
                gguf_to_hf_name_map[f"blk.{idx}.ssm_beta.weight"] = (
                    f"model.layers.{idx}.ssm_beta.weight"
                )
                gguf_to_hf_name_map[f"blk.{idx}.ssm_conv1d.weight"] = (
                    f"model.layers.{idx}.ssm_conv1d.weight"
                )
                gguf_to_hf_name_map[f"blk.{idx}.ssm_dt.bias"] = (
                    f"model.layers.{idx}.ssm_dt.bias"
                )
                gguf_to_hf_name_map[f"blk.{idx}.ssm_norm.weight"] = (
                    f"model.layers.{idx}.ssm_norm.weight"
                )
                gguf_to_hf_name_map[f"blk.{idx}.ssm_out.weight"] = (
                    f"model.layers.{idx}.ssm_out.weight"
                )
                # attn_gate is the gating mechanism for SSM
                gguf_to_hf_name_map[f"blk.{idx}.attn_gate.weight"] = (
                    f"model.layers.{idx}.attn_gate.weight"
                )
            # MTP (nextn) layer mapping - layer 64 in GGUF corresponds to MTP
            # The MTP layer has nextn_predict_layers config
            mtp_layer_idx = config.num_hidden_layers  # typically layer 64 (0-63 main + 64 MTP)
            gguf_to_hf_name_map[f"blk.{mtp_layer_idx}.nextn.shared_head_norm.weight"] = (
                f"model.layers.{mtp_layer_idx}.nextn.shared_head_norm.weight"
            )
            gguf_to_hf_name_map[f"blk.{mtp_layer_idx}.nextn.enorm.weight"] = (
                f"model.layers.{mtp_layer_idx}.nextn.enorm.weight"
            )
            gguf_to_hf_name_map[f"blk.{mtp_layer_idx}.nextn.hnorm.weight"] = (
                f"model.layers.{mtp_layer_idx}.nextn.hnorm.weight"
            )
            gguf_to_hf_name_map[f"blk.{mtp_layer_idx}.nextn.eh_proj.weight"] = (
                f"model.layers.{mtp_layer_idx}.nextn.eh_proj.weight"
            )
            # Skip MTP layers in sideload_params
            sideload_params.append(
                re.compile(f"blk\\.{mtp_layer_idx}\\.nextn\\..*")
            )
